...
# stages a synthetic per-turn transcript under ~/.gemini/antigravity/brain/ and
...
ID="agbwtest$$"
BRAIN="$HOME/.gemini/antigravity/brain/$ID"
...
check "entrypoint is antigravity-app"  '[ "$(state entrypoint)" = "antigravity-app" ]'

# 2. same line quiet past the 6s threshold -> the app is sitting on its prompt
